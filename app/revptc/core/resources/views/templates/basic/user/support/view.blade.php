                    ...
                    <div class="card-body">
                        <form method="post" class="disableSubmission" action="{{ route('ticket.reply', $myTicket->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="row justify-content-between">
                                ...
                                <div class="col-md-9">
                                    <button type="button" class="btn {{ auth()->user() ? 'btn-dark' : 'btn--base' }} btn-sm addAttachment "> <i class="fas fa-plus"></i> @lang('Add Attachment') </button>
                                    <p class="mb-2"><span class="text--info">@lang('Max 5 files can be uploaded | Maximum upload size is '.convertToReadableSize(ini_get('upload_max_filesize')) .' | Allowed File Extensions: .jpg, .jpeg, .png, .pdf, .doc, .docx')</span></p>
                                    <div class="row fileUploadsContainer">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <button type="submit" class="btn {{ auth()->user() ? 'btn--primary' : 'btn--base' }}  w-100 "> <i class="fa fa-reply"></i>
                                        @lang('Reply')</button>
                                </div>
